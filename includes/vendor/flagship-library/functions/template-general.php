...
function flagship_get_post_navigation( $args = array() ) {
	...
	$defaults = apply_filters( 'flagship_get_post_navigation_defaults',
		array(
			...
			'prev_text'      => __( 'Previous', 'woodd' ) . esc_attr( $name ),
			'next_text'      => __( 'Next', 'woodd' ) . esc_attr( $name ),
			'in_same_term'   => false,
			'excluded_terms' => '',
			'taxonomy'       => 'category',
		)
...
function flagship_get_posts_navigation( $args = array() ) {
	...
	$defaults = apply_filters( 'flagship_loop_nav_defaults',
		array(
			'format'         => 'pagination',
			'prev_text'      => sprintf( '<span class="screen-reader-text">%s</span>' , __( 'Previous Page', 'woodd' ) ),
			'next_text'      => sprintf( '<span class="screen-reader-text">%s</span>', __( 'Next Page', 'woodd' ) ),
			'prev_link_text' => __( 'Newer Posts', 'woodd' ),
			'next_link_text' => __( 'Older Posts', 'woodd' ),
		)
	);
...
function flagship_get_credit_link() {
	...
	$title = sprintf( __( 'Purpose-Built WordPress Theme by %s', 'woodd' ), $name );
